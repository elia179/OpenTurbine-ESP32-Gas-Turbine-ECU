...
- Keep sequencer cards and output ownership consistent: one glow-plug control path, hardware-owned On level and ramp, and no separate igniter preheat setting. Relay outputs show no ramp controls; wet-glow pilot fuel follows its glow-plug command after the configured delay.
- Align Hardware editor controls and group servo/ESC electrical pulse endpoints together, including on narrow screens.
- Clarify unavailable ignition options, hardware ramp behavior, and sequence output targets in the UI.
