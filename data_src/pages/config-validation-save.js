...
function runValidation() {
  ...
  if (hasRegistryInput('n1_speed') && computedPreIgnRpm !== undefined && computedSpoolRpmTarget !== undefined
  ...
  const inlineIdleSourceAvailable = [hasRegistryInput('n1_speed'), hasRegistryInput('n2_speed'),
    hasRegistryInput('p1_pressure'), hasRegistryInput('p2_pressure')][idleSource] === true;
  if (hwCfg.controllers?.dynamic_idle && inlineIdleSourceAvailable &&
      idleTargetRpm !== undefined && idleComparisonLimit !== undefined &&
  ...
  if (hasRegistryInput('n1_speed') && n1PullbackEnabled && rpmLimit > 0 &&
  ...
  const inlineStandbySourceAvailable = standbySourceNow === 0 ? hasRegistryInput('n1_speed') :
    standbySourceNow === 1 ? hasRegistryInput('n2_speed') :
    (hasRegistryInput('n1_speed') || hasRegistryInput('n2_speed'));
  if (standbyOilEnabledNow && inlineStandbySourceAvailable && usableStandbyLimitsNow.length && usableStandbyLimitsNow.every(limit => standbyRpmNow >= limit)) {
  ...
  if (standbyOilEnabledNow && inlineStandbySourceAvailable && (fv('so_fp') || 0) <= 0 && (fv('so_fb') || 0) <= 0) {
  ...
  if (hasActualAfterburnerHardware() && abMainOffsetNow < 0 && mainFuelMinimumNow > 0) {
  ...
  if (hasOilPressure && oilRunningVal !== undefined && oilRunningVal === 0) {
    warnings.push({
...
async function validateBeforeSave(cfg) {
  ...
  if (hwCfg.controllers?.dynamic_idle && idleSource < 2 && idleSourceAvailable &&
      hasAnyIdleRpm && idleTarget !== undefined && minRpm !== undefined && idleTarget < minRpm)
  ...
  if (hwCfg.safety?.n2_overspeed && Number(n2RpmLimit) > 0) {
    ...
    if (hwCfg.cluster_serial?.enabled && hasN2 && clusterN2Warn > 0 && clusterN2Warn >= n2RpmLimit)
  ...
  if (hasN1 && pbN1Enabled && Number(rpmLimit) > 0 &&
  ...
  const standbySourceAvailable = standbySource === 0 ? hasN1 :
                                 standbySource === 1 ? hasN2 : (hasN1 || hasN2);
  if (standbyEnabled && standbySourceAvailable && usableLimits.length && usableLimits.every(limit => standbyRpm >= limit))
  ...
  if (standbyEnabled && standbySourceAvailable && standbyPct <= 0 && standbyBar <= 0)
  ...
  if (hasActualAfterburnerHardware() && abPmn !== undefined && abPmx !== undefined && abPmx < abPmn)
  ...
  if (hasActualAfterburnerHardware() && abMainOffset < 0 && mainFuelMinimum > 0)
  ...
  const pbPairs = [
    ['pullback_n1', hasN1, 'pullback_n1_soft_rpm', 'pullback_n1_hard_rpm', 'N1 Pullback Start', 'N1 Pullback Full'],
    ['pullback_n2', hasN2, 'pullback_n2_soft_rpm', 'pullback_n2_hard_rpm', 'N2 Pullback Start', 'N2 Pullback Full'],
    ['pullback_egt', hasTotHw || hasTitHw, 'pullback_egt_soft_c', 'pullback_egt_hard_c', 'EGT Pullback Start', 'EGT Pullback Full'],
    ['pullback_p1', hasP1, 'pullback_p1_soft_bar', 'pullback_p1_hard_bar', 'P1 Pullback Start', 'P1 Pullback Full'],
    ['pullback_p2', hasP2, 'pullback_p2_soft_bar', 'pullback_p2_hard_bar', 'P2 Pullback Start', 'P2 Pullback Full'],
    ['pullback_torque', hasRegistryInput('torque'), 'pullback_torque_soft_nm', 'pullback_torque_hard_nm', 'Torque Pullback Start', 'Torque Pullback Full'],
  ];
  pbPairs.forEach(([enabledKey, hardwareAvailable, softKey, fullKey, l1, l2]) => {
    const enabled = !!gv(cfg, 'throttle', enabledKey);
    const soft = gv(cfg, 'throttle', softKey), full = gv(cfg, 'throttle', fullKey);
    if (hardwareAvailable && enabled && soft !== undefined && full !== undefined && soft > 0 && full > 0 && full <= soft)
  ...
  if (errors.length) {
    await OTDialog.alert('Cannot save — fix these errors:\n\n' + errors.map(e => '• ' + e).join('\n'), {
      title:'Settings need attention', links:OTValidationLinks(errors)
    });
    return false;
  }
  if (warns.length) {
