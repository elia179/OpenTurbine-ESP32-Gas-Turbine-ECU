...
Use **Edit cards** to hide unneeded cards or follow a card's limit-setting link. **Arrange cards** switches from the default named groups to one **Your dashboard** section: drag cards by their grip, or focus a grip and use the up/down arrow keys. Start/Stop, engine state, and fault banners stay outside the movable section. **Restore default layout** restores the named groups, their original card order, and all hidden cards. Card layout is saved only in this browser, not in the ECU's engine-file backup; it does not change firmware behavior.
